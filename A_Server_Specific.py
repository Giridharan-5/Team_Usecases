...
# ----------------------------
    server_label_to_id = {
        item["label"]: item["vm_id"] for item in server_data
    }
    server_labels = [
       item["label"] for item in server_data
    ]

# ----------------------------
# MULTI-SERVER SELECT (REPLACES selectbox)
# ----------------------------
    col_server, col_resource = st.columns(2)

    selected_servers = col_server.multiselect(
        label="Select Hosts:",
        options=server_labels,
        placeholder="Choose one or more hosts...",   
        key="active_server",
    )

# resolve selected labels → vm_ids
    resolved_server_ids = [server_label_to_id[label] for label in selected_servers 
                           if label in server_label_to_id]
    print("--server ids---",resolved_server_ids)
...
# ----------------------------
# MULTI-RESOURCE SELECT
# ----------------------------
    if api_base_clean and resolved_server_ids:
        try:
        # fetch resources for first selected server (or modify backend for multi-server support)
            resource_options = fetch_resource_options(
                api_base_clean,
                resolved_server_ids[0]
            )
        except Exception as exc:
            st.warning(f"Could not load resources: {exc}")
            resource_options = []
    else:
        resource_options = []
    ...
    selected_resources = col_resource.multiselect(
        label="Select Resource Type:",
        options=resource_choices,
        placeholder="Choose one or more hosts...",        
        key="active_resource",
    )
    resolved_resources = selected_resources
    # ============================
    # BUILD BULK JOB LIST
    # ============================
    ...
    jobs = [
        {
            "server_id": resolved_server_ids,
            "resource_type": resolved_resources
        }
        for server_id in resolved_server_ids
        for resource in selected_resources
    ]
    for job in jobs:
        print(job["server_id"])
        print(job["resource_type"])
    use_bulk_pipeline = len(jobs) > 1
    print("--jobs---",len(jobs),"--use_bulk_pipeline---",use_bulk_pipeline)

    stored_server = st.session_state.get("selected_server", [])
    stored_resource = st.session_state.get("selected_resource", [])
    print("--stored values---",stored_server,stored_resource)
    
    if st.session_state.get("pipeline_result") and (
        resolved_server_ids != stored_resource
        or resolved_resources != stored_resource
    ):
    ...
    with col_run:
        if st.button(
            ...
            def unwrap(value):
                    # Keep taking the first item as long as the value is still a list
                    while isinstance(value, list):
                        if len(value) > 0:
                            value = value[0]
                        else:
                            return None  # Handles empty lists like [] safely
                    return value
            with st.spinner("Running LangGraph pipeline..."):
                try:
                    ...
                    for job in jobs:
                        ...
                        # Clean it using Method 2
                        cleaned_payload = {key: unwrap(value) for key, value in payload.items()}
                        #server_id= unwr(job["server_id"]),
                        #resource_type = eval(job["resource_type"])
                        print(cleaned_payload)
                        # AUTO ROUTE PIPELINE
                        if use_bulk_pipeline:
                            url = f"{api_base_clean.rstrip('/')}/pipeline"
                        else:
                            url = f"{api_base_clean.rstrip('/')}/pipeline"
                    
                        result = api_post(url, cleaned_payload)
                        all_results.append(result)
                    # STORE CLEAN STATE
                    st.session_state["pipeline_result"] = all_results
    ...
    for item in pipeline_result or []:
        if isinstance(item, dict):
            rec = item.get("recommendation")
            if rec:
                recommendations.append(rec)

    # choose first recommendation (or you can merge later)
    ...
    if approval_result:
        ...
        for item in pipeline_result:
            display_cost_summary(item.get("cost_impact") or {})
